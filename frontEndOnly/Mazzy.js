...
let movePlayer = (newRowIndex, newColIndex) => {
  if (newRowIndex < 0 || newRowIndex >= map.length ||
    newColIndex < 0 || newColIndex >= map[0].length || map[newRowIndex][newColIndex] === "W")
    {
  ...
  console.log(`what the eff ${map[player.rowIndex][player.colIndex]}`)
  ...
  console.log(` ok ?${map[newRowIndex][newColIndex]}`)
...
renderMaze()
console.log(player)
//farmers are hacking their tractors
